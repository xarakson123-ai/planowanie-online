...
function stateFor(room,pid){const p=room.players.find(x=>x.id===pid);return {room:publicRoom(room),myId:pid,hand:(p?.hand||[]).map(c=>({id:c.id,key:c.key,s:c.s,r:c.r}))};}
...
function play(room,pid,card){
  const pl=p(room,pid);
  if(!pl||room.phase!=='play'||room.currentPlayer!==pid)return 'Nie jest teraz Twoja kolej.';

  // The suit + rank pair is the canonical identity of a card in a standard deck.
  // This prevents stale DOM IDs / reordered hands from causing "Nie masz tej karty".
  const s=typeof card?.s==='string'?card.s:null;
  const r=typeof card?.r==='string'?card.r:null;
  let idx=-1;
  if(s && r) idx=pl.hand.findIndex(c=>c.s===s && c.r===r);
  if(idx<0 && card?.key) idx=pl.hand.findIndex(c=>c.key===card.key);
  if(idx<0 && card?.id) idx=pl.hand.findIndex(c=>c.id===card.id);

  if(idx<0){
    console.log('[PLAY REJECT]', room.code, pl.name, 'sent=', JSON.stringify(card),
      'hand=', pl.hand.map(c=>c.s+c.r).join(','));
    return 'Nie masz tej karty.';
  }
